...
**`foist` can write lossless grey image files ~5.5x faster than the `png` library.**

**`foist` can write lossless RGB image files ~7.5x faster than the `png` library.**
...
* [viridis](https://cran.r-project.org/package=viridis) - Wnderful palettes originally from [matplotlib](http://matplotlib.org).
...
* A palette must be an integer matrix with dimensions 256 x 3 and values in the
  range [0, 255].
* Pixel values in the matrix are first scaled into the range [0, 255] and 
